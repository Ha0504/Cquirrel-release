...
import Q1Validation from "./components/Q1Validation";
// import QueryFig from "./components/QueryFig"
import Settings from "./components/Settings";
import About from "./components/About";
import RelationGraph from "./components/RelationGraph"
import {BACKEND_ORIGIN, BACKEND_WS_ORIGIN} from "./config";
import axios from 'axios';
import {Layout, Menu, Row, Col, message, Card, Button, Input, InputNumber, Divider, Spin, Empty} from 'antd';
...
class App extends Component {
    ...
    applyInformationData = (informationData) => {
        try {
            const parsed = typeof informationData === 'string' ? JSON.parse(informationData) : informationData;
            this.setState({
                relationsData: parsed,
                relationGraphLoading: false,
                showRelationGraph: true,
                flowDiagData: parsed.relations || [],
                flowDiagLoading: false,
                showFlowDiag: true,
            });
        } catch (error) {
    ...
    applyCodegenLog = (codegenLog, retcode = 0) => {
        this.setState({
            codegenLogLoading: false,
            codegen_log: codegenLog || '',
            cur_step: retcode === 0 ? 3 : 1,
        });
    };

    applyQueryResultSnapshot = (snapshot) => {
        const nextChartOption = {
            ...this.state.chart_option,
            title: {
                ...this.state.chart_option.title,
                text: snapshot.chart_title || 'Result Chart',
            },
            legend: {
                ...this.state.chart_option.legend,
                data: snapshot.legend_data || [],
            },
            xAxis: {
                ...this.state.chart_option.xAxis,
                data: snapshot.x_axis_data || [],
            },
            yAxis: {
                ...this.state.chart_option.yAxis,
                name: snapshot.y_axis_name || this.state.chart_option.yAxis.name,
            },
            dataZoom: {
                ...this.state.chart_option.dataZoom,
                startValue: snapshot.data_zoom_start_value || 1,
            },
        ...
        if (chartDom) {
            ...
            chart.setOption(nextChartOption, true);
        }

        this.setState({
            table_cols: snapshot.table_cols || [],
            table_data: snapshot.table_data || [],
            table_title: snapshot.table_title || 'TPC-H Query Result Table: ',
            queryResultMessage: snapshot.message || '',
            chart_option: nextChartOption,
            queryTableLoading: false,
            queryChartLoading: false,
        ...
        if (snapshot.empty) {
            message.info(snapshot.message || 'Query finished with no rows.');
        }
    };

    fetchQ1Validation = () => {
        axios.get(`${BACKEND_ORIGIN}/r/q1_validation`).then((res) => {
            const payload = res.data || {};
            this.setState({
                q1ValidationLoading: false,
                q1Validation: payload.ok ? payload : null,
                q1ValidationMessage: payload.ok ? '' : (payload.error || 'Q1 validation is unavailable.'),
            });
        }).catch((err) => {
            ...
            this.setState({
                q1ValidationLoading: false,
                q1Validation: null,
                q1ValidationMessage: String(detail),
            });
        });
    };
    ...
    pollJobUntilComplete = (jobId) => {
        ...
        const pollOnce = () => {
            axios.get(`${BACKEND_ORIGIN}/r/job_status`, {
                params: {job_id: jobId},
            }).then((statusRes) => {
                const state = String(statusRes.data?.state || '').toUpperCase();
                if (state === 'FINISHED') {
                    this.stopJobPolling();
                    axios.get(`${BACKEND_ORIGIN}/r/query_result_snapshot`).then((snapshotRes) => {
                        this.applyQueryResultSnapshot(snapshotRes.data || {});
                        this.fetchQ1Validation();
                    }).catch((err) => {
                ...
                } else if (['FAILED', 'CANCELED', 'CANCELLING'].includes(state)) {
                    this.stopJobPolling();
                    message.error(`Flink job ended with state ${state}`);
                    this.setState({
                        queryTableLoading: false,
                        queryChartLoading: false,
                        q1ValidationLoading: false,
        ...
        pollOnce();
        this.jobPollTimer = window.setInterval(pollOnce, 1500);
    };
    ...
    build = () => {
        ...
        // init websocket
        const _socket = io(BACKEND_WS_ORIGIN, {
            transports: ['polling']
        });
    ...
    handleSubmitSql = (v) => {
        ...
        axios.post(`${BACKEND_ORIGIN}/r/submit_sql`, {sql: v}).then(res => {
            const payload = res.data || {};
            console.log(payload);

            if (payload.information_data) {
                this.applyInformationData(payload.information_data);
            } else {
                this.setState({
                    relationGraphLoading: false,
                    flowDiagLoading: false,
            ...
            this.applyCodegenLog(payload.codegen_log, payload.retcode);
            this.pollJobUntilComplete(payload.job_id);
        }).catch(err => {
            const detail = err.response?.data || err.message;
            message.error(detail);
            this.setState({
                            ...
                            <div className="site-layout-background" style={{padding: 24, minHeight: 640}}>
                                ...
                                <Row>
                                    ...
                                    <Col span={18}>
                                            ...
                                            <Col span={24}>
                                                <Spin tip={"loading"} spinning={this.state.queryTableLoading}>
                                                    <QueryTable table_cols={this.state.table_cols}
                                                                ...
                                                                emptyText={this.state.queryResultMessage || 'No data'}
                                        ...
                                        <Row>
                                            <Col span={24}>
                                                <Spin tip={"loading"} spinning={this.state.q1ValidationLoading}>
                                                    <Q1Validation
                                                        validation={this.state.q1Validation}
                                                        message={this.state.q1ValidationMessage}
                                                    />
                                                </Spin>
                                            </Col>
                                        </Row>
                                                ...
                                                <Spin tip={"Loading"} spinning={this.state.queryChartLoading}>
                                                    <div className="queryfig-card">
                                                        ...
                                                        <Card title="Query Result Figure: " extra={
                                                            ...
                                                            {!this.state.queryChartLoading &&
                                                                (!this.state.chart_option.series || this.state.chart_option.series.length === 0) &&
                                                                this.state.queryResultMessage && (
                                                                    <div style={{paddingBottom: 16}}>
                                                                        <Empty
                                                                            image={Empty.PRESENTED_IMAGE_SIMPLE}
                                                                            description={this.state.queryResultMessage}
                                                                        />
                                                                    </div>
                                                                )}
