...
def data_generator(config_file_name):
    ...
    if kafka_enable:
        if Producer is None:
            raise ImportError("confluent_kafka is required when KafkaEnable = yes")
    ...
    lineitem_size = max(1, int(scale_factor * 6000000))
    orders_size = max(1, int(scale_factor * 1500000))
    nation_size = 25
    region_size = 5
    partsupp_size = max(1, int(scale_factor * 800000))
    part_size = max(1, int(scale_factor * 200000))
    supplier_size = max(1, int(scale_factor * 10000))
    customer_size = max(1, int(scale_factor * 150000))
    delete_enabled = window_size < lineitem_size
    ...
    while line_lineitem and count < lineitem_size:
        count = count + 1
        delete_count = delete_count + 1
        if isLineitem:
            write_to_output_and_kafka("+LI" + line_lineitem)
        line_lineitem = lineitem.readline()
        if delete_enabled and delete_count > 0:
            if isLineitem:
        ...
        if count * orders_size / lineitem_size > orders_count and line_orders:
            ...
            line_orders = orders.readline()
            if delete_enabled and delete_count * orders_size / lineitem_size > orders_delete_count and line_orders_d:
        ...
        if count * customer_size / lineitem_size > customer_count and line_customer:
            ...
            line_customer = customer.readline()
            if delete_enabled and delete_count * customer_size / lineitem_size > customer_delete_count and line_customer_d:
        ...
        if count * part_size / lineitem_size > part_count and line_part:
            ...
            line_part = part.readline()
            if delete_enabled and delete_count * part_size / lineitem_size > part_delete_count and line_part_d:
        ...
        if count * supplier_size / lineitem_size > supplier_count and line_supplier:
            ...
            line_supplier = supplier.readline()
            if delete_enabled and delete_count * supplier_size / lineitem_size > supplier_delete_count and line_supplier_d:
        ...
        if count * partsupp_size / lineitem_size > partsupp_count and line_partsupp:
            ...
            line_partsupp = partsupp.readline()
            if delete_enabled and delete_count * partsupp_size / lineitem_size > partsupp_delete_count and line_partsupp_d:
        ...
        if count * nation_size / lineitem_size > nation_count and line_nation:
            ...
            line_nation = nation.readline()
            if delete_enabled and delete_count * nation_size / lineitem_size > nation_delete_count and line_nation_d:
        ...
        if count * region_size / lineitem_size > region_count and line_region:
            ...
            line_region = region.readline()
            if delete_enabled and delete_count * region_size / lineitem_size > region_delete_count and line_region_d:
    ...
    while delete_enabled and line_lineitem_d and delete_count < lineitem_size:
        delete_count = delete_count + 1
        if isLineitem:
            write_to_output_and_kafka("-LI" + line_lineitem_d)
        line_lineitem_d = lineitem_d.readline()
